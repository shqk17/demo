...
$.fn.extend({
    ...
    ajaxPage: function (options) {
        ...
        var ajaxSuc = function (data) {
            if (data) {
                ...
                var
                    //总数量
                    allCount = setting.pageData ? eval(setting.pageData) : (data.Total),
                    //总页数
                    allPage = 0; 
                //如果能取到总数量并且是大于0整数
    ...
    pagesFn: function (setting) {
        var
            ...
            //分页按钮html容器
            pagehtml = '<a ' + (isfirstpg ? 'class="paginate_button"' : 'class="paginate_button"') + ' href="javascript:;" pgid="-3">' + btnTxt.first + '</a>' +
        ...
        pagehtml += '<a ' + (lastpg ? 'class="last paginate_button paginate_button_disabled"' : 'class="last paginate_button"') + ' href="javascript:;" pgid="-1">' + btnTxt.next + '</a>' +
                                 '<a ' + (lastpg ? 'class="paginate_button"' : 'class="paginate_button"') + ' href="javascript:;" pgid="-2">' + btnTxt.last + '</a><span class="pagespan">跳转到:</span><input type="text" class="pageipt js_pageipt"/><span class="paginate_button js_gopages"></span>';
